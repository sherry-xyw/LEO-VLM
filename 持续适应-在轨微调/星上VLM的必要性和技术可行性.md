...
持续适应的必要性主要来自三类变化：观测场景和成像条件导致的分布迁移、任务对象与能力边界导致的类别和任务拓展，以及外部知识与业务规则的持续更新。三者对应的失效机理不同，因此不能采用单一更新方式处理。
...
CLeaRS 的长时程、模态增量和任务增量评测表明，传统持续学习策略不能简单外推到 RS-VLM：任务、指令和模态变化会同时影响跨模态对齐、任务路由与旧能力保持 [1]。通用 MLLM 的连续学习研究也显示，模型面对新领域与新能力时会发生灾难性干扰，需要结合参数隔离和任务路由来维持旧能力 [6]；而模态不一致持续学习研究进一步指出，模态类型和任务类型同时变化时，遗忘会比单一类别增量更严重 [7]。
...
现有遥感增量目标检测研究已围绕“以少量新类样本学习新类别，同时保持基类性能”开展探索。例如，InfRS 利用类别原型与校准机制缓解新旧类别间遗忘，并在 NWPU VHR-10 与 DIOR 上完成验证 [8]。这类工作虽然面向专用检测模型，而非 VLM，但说明当任务需要结构化、高置信输出时，新类别仍需要基于样本、原型和局部参数更新进行适配。
...
综上，三类变化分别指向运行状态校准、外部知识更新与局部能力学习；在多星场景下，还需要进一步考虑更新的协同聚合和长期经验积累。下节据此按更新对象和系统尺度，依次讨论五条技术路线。

---

## 3. 技术路线

以下路线沿“运行状态—外部知识—局部参数—星座协同—长期演化”递进展开：前两类主要在不改变或极少改变模型参数的条件下完成适应，后三类则逐步引入参数更新、跨节点协同与反馈驱动的能力积累。该组织方式对应更新成本、遗忘风险和系统复杂度的逐步上升。

### 技术一：漂移检测 + 轻量自监督/测试时适应

**技术内涵：** 漂移检测是指持续比较当前在轨观测流与模型部署时的参考分布，以判断模型输入、内部表征或任务输出是否发生了足以影响可靠性的变化。其检测对象可分为三类：一是输入分布漂移，如云雾、季节、太阳高度、观测几何和噪声造成的像素或谱段统计变化；二是特征与跨模态漂移，如视觉特征分布、图像—文本相似度和类别原型覆盖度发生偏移；三是任务表现漂移，如置信度校准变差、结构化输出错误增多或告警漏检/虚警上升。漂移检测本身不直接学习新能力，而是为后续“是否适应、适应什么、是否升级至参数更新”提供触发依据。

轻量自监督适应与测试时适应（test-time adaptation, TTA）是在当前未标注或弱标注观测流上进行局部校准的两类方法。轻量自监督通常利用掩码重建、时相一致性、多视角一致性或高置信样本等代理目标，维持视觉表征对当前观测条件的适配；TTA 则在推理阶段或一个任务窗口内，利用熵最小化、原型校正、伪标签一致性等目标更新少量状态。二者均强调只调整归一化统计量、类别原型、温度参数、Prompt 或极小 Adapter 等低风险对象，并保持基座模型主体不变；其中，自监督适应更偏向连续表征校准，TTA 更偏向短时、可复位的运行时校准。
...
该路线的完整机制包括知识采集、表征编码、索引构建、检索召回、重排序与时空过滤、上下文注入以及规则校验。其关键不在于简单增加记忆数量，而在于为每条知识或案例建立来源、时间、地域、传感器、权限、置信度、版本和有效期等元数据，并能够处理过期知识、相互矛盾证据和检索错误。由于更新对象位于模型外部，记忆和规则可独立签名、增量同步、删除与回滚，具有较好的可控性。
...
对于 VLM，PEFT 的核心不是“少训练一些参数”，而是把更新模块与变化来源对应起来：视觉域失配主要影响视觉编码器或视觉 Adapter；新模态或跨模态对齐问题主要影响投影层、连接器和模态 Adapter；任务指令、输出格式或语言推理方式变化则更多涉及 Prompt、任务头与语言侧 LoRA。持续学习场景下，还需通过任务专属 Adapter/LoRA bank、模块路由、代表样本或原型回放、旧版本蒸馏和新旧任务联合评测，降低不同增量模块之间的干扰与灾难性遗忘。
...
- LLaMA-Adapter V2 进一步将参数高效适配扩展到视觉指令模型，说明视觉 token 融合、跨模态对齐和指令能力可以通过少量新增参数进行针对性调整 [18]。
- TPPT 利用稳定的文本原型引导视觉与文本 Prompt 的增量学习，体现了“以语言语义锚点约束跨模态表征漂移”的轻量持续学习思路 [9]。
...
**技术内涵：** VLM 自进化不是单一算法，而是一种利用持续到来的观测、任务反馈和外部监督信号逐步积累经验、构造增量监督并改进模型的长期适应范式。它并非与前四条路线相互替代，而是将前述记忆、轻量适应、PEFT 与协同更新串联为“反馈获取—经验沉淀—候选更新—验证发布”的长期闭环。其基本链路包括：从未标注观测、人工审核、多时相一致性、跨传感器一致性和任务结果中提取候选经验；对候选经验进行质量评估、冲突检测与可信筛选；优先写入案例记忆和检索索引；在满足数据质量、覆盖度和安全要求后，再将部分可信样本用于训练局部 PEFT 候选版本；最后通过新旧任务、资源与风险联合验证决定是否发布。

“受控”意味着模型自身的生成结果不能直接被视为长期训练标签，更不能自动替换稳定版本。自进化必须建立在来源可追溯、监督信号可验证、候选更新可回滚和发布前可评测的条件上。其重点是把模型输出转化为经过筛选的经验资产，而不是形成“自生成数据—自主训练—自动部署”的无约束闭环；在星上场景中，案例记忆、错误样本队列和可信度统计通常应先于参数更新发生。
...
第 2 节说明“为什么需要更新”，第 3 节说明“可采用哪些更新机制”；本节将二者映射为面向任务变化的选择规则，用于确定首选路径、参数更新升级条件及应避免的更新方式。
